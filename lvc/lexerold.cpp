...
bool Lexer::addDedentsToQueueUntilColnumberIsReached(colnumber col) {
    ...
    if (col != currentCol) {
        issueReporter.issue(reader.getRow(), reader.getCol(), ErrorInvalidDedent, SubsystemLexer);
        return false;
    }
...
void Lexer::makeIndentToken(Token &tok, colnumber col) {
    assert(col > indentationStack.top());
    
    colnumber oldIndent = indentationStack.top();
    indentationStack.push(col);
    
    tok.setRow(reader.getRow());
    tok.setStartCol(oldIndent);
    tok.setLength(col - oldIndent);
    tok.setKind(Indent);
}

#warning TODO: make this use exceptions for unrecoverable exceptions and document it
...
bool Lexer::readNextToken(Token &tok) {
    ...
    if (atStartOfRowBeforeConsumingWhitespace) {
        ...
        if (indentationLength > indentationStack.top()) {
            makeIndentToken(tok, indentationLength);
            return true;
        }
...
#warning: (comments) not these sequences of characters are not interpreted as comment delimiters if they appear inside string or character literals.
    ...
    if (c == '/' && reader.peekChar() == '*') {
        ...
        issueReporter.issue(rowOfC, colOfC, ErrorUnterminatedBlockComment, SubsystemLexer);
        return false;
    }
    ...
    if (c == '\r') {
        if (reader.peekChar() != '\n') {
            issueReporter.issue(rowOfC, colOfC, ErrorInvalidNewline, SubsystemLexer);
...
#warning TODO: fix this; make it just read the whole damn token thing.
        
        if (decimalPoints > 1) {
            issueReporter.report(rowOfC, colOfC, LexerErrorInvalidNumberLiteral);
            
            // Recover from error by just giving back a 0.0 token
            tok.setKind(RealLiteral);
            tok.setStr("0.0");
            tok.setLength(std::string("0.0").length());
            return true;
        }
        
#warning TODO: should skip line instead
