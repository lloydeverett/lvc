...
public:
   ...
   // llvm::Value* genGlobalDecl(const ast::VariableDecl& variableDecl) {
    ...
    llvm::Value* genCast(llvm::Value* val, ImplicitCast& cast) {
        #warning TODO: cast is different for signed/unsigned zext
        switch (cast.kind) {
            case CastKindNullCast:
                return val;
            case CastKindIntegerUpcast:
            {
                ast::IntegerType* iType = dynamic_cast<ast::IntegerType*>(cast.getTypePtr());
                if (iType->isSigned) {
